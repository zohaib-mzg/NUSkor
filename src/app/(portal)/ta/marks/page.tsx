...
import { calculateAssessmentStats } from "@/lib/marks";
  ...
  return (
    <div>
      ...
      {!assessmentId ? (
        ...
        <>
          {/* Class stats bar */}
          {rows.length > 0 && selectedAssessment && (() => {
            const stats = calculateAssessmentStats(
              rows.length,
              rows.map((r) => r.saved),
              selectedAssessment.total_marks
            );
            if (stats.markedCount === 0) return null;
            return (
              <div className="mb-4 grid gap-3 sm:grid-cols-4">
                ...
                <div className="rounded-xl bg-green-50 p-4">
                  ...
                  {stats.highest !== null ? (
                    <p className="mt-1 text-xl font-extrabold text-ink">
                      {stats.highest} <span className="text-sm font-normal text-ink/30">/ {stats.maxMarks}</span>
                    </p>
                  ) : (
                    <p className="mt-1 text-xl font-extrabold text-ink">N/A</p>
                  )}
                ...
                <div className="rounded-xl bg-red-50 p-4">
                  ...
                  {stats.lowest !== null ? (
                    <p className="mt-1 text-xl font-extrabold text-ink">
                      {stats.lowest} <span className="text-sm font-normal text-ink/30">/ {stats.maxMarks}</span>
                    </p>
                  ) : (
                    <p className="mt-1 text-xl font-extrabold text-ink">N/A</p>
                  )}
                ...
                <div className="rounded-xl bg-blue-50 p-4">
                  ...
                  <p className="mt-1 text-xl font-extrabold text-ink">
                    {stats.markedCount} <span className="text-sm font-normal text-ink/30">/ {stats.totalStudents}</span>
                  </p>
                  <p className="text-xs font-semibold text-blue-600">{stats.markedPercentage.toFixed(1)}%</p>
                </div>
              </div>
            );
          })()}
